...
We would expect to see both graphs of eq size (50% on each side) if the two are independent. They do not appear to be equal ... shedding doudt on the idea of independence.  
...
We now want to run a statistical test to see if the aides vaccine was seemingly effective. We state the null and alterative Hypothesis for this test.  

$H_0:$ There is no relationship between getting aids and having taken the aids vaccine  

$H_a:$ There is a relation between aids and having the aides vaccine.  
```{r}
...
we now test the independence of aids vacine and placebo groups  
